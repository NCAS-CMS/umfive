...
def test_umdump_matches_core_variable_names_with_p5dump_fixture():
    pp_out = _run_umdump("tests/data/test2.pp")
    ...
    # umfive fixture has no bounds variables, but core variables should align.
    assert expected_core.issubset(pp_vars)
    assert expected_core.issubset(p5_vars)


def test_umdump_help_and_special_flag():
    assert umdump_main(["-h"]) == 0
    assert umdump_main(["-s", "tests/data/test2.pp"]) == 0


def test_umdump_time_axis_metadata_matches_expected():
    out = _run_umdump("tests/data/test2.pp")
